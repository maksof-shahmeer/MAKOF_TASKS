...
// console.log(typeof ({ name : 'shahmeer', course : 'BSCS', semester : "3rd" })) //object
...
const passedStudents = students.reduce((accumulator, student) => {
    if (student.score >= 80) {
      accumulator.total += student.score;
      accumulator.count++;
    }
    return accumulator;
  }, { total: 0, count: 0 });
  
  const averageScore = passedStudents.total / passedStudents.count;
  console.log(averageScore); // Output: 87.5
  let score = [23,24,26,28,45,90,50];
  let evenNumber = score.filter((scores) => scores % 2 === 0
  ) 
  console.log(evenNumber);
  let scores = [23,24,26,28,45,90,50];
  let evenNumbers = score.find((scores) => scores % 2 === 0
  ) 
  console.log(evenNumbers);

  kReduce = score.reduce((accumulator, scores, index ) => {return  [scores] },0);
  console.log(checkReduce);
  let chec
  console.log(a=score.toString());
...
const iterators = fruits.keys();

for (const index of iterators) {
  console.log(index); // Output: 0, 1, 2
}

const iteratorss = fruits.entries();

for (const entry of iteratorss) {
  console.log(entry);
}
for ( let gg of score) {
  console.log(gg);
...
console.log(numbers); // Original array is modified: [1, 6, 7, 4, 5]
